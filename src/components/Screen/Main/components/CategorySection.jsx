          ...
          return (
            <li key={category.key} className="category_row">
              ...
              <div className="category_right">
                {!isEditMode ? (
                  <b className="category_value">{formatMaskedKoreanWon(category.amount)}</b>
                ) : (
                  <input
                    className="category_input"
                    inputMode="numeric"
                    value={categoryAmountInput[category.key] ?? ""}
                    onChange={(event) => onCategoryChange(category.key, event.target.value)}
                    onBlur={() => onCategoryCommit(category.key)}
                    onKeyDown={(event) => onCategoryEnter(event, category.key)}
                    aria-label={`${category.label} 금액 수정`}
                  />
                )}
